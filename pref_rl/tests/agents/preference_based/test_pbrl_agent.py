...
def agent(cartpole_env):
    return PbRLAgent(policy_model=MagicMock(),
                     reward_model=MagicMock(),
                     pretraining_query_generator=MagicMock(),
                     query_generator=MagicMock(),
                     preference_querent=MagicMock(),
                     preference_collector=MagicMock(),
                     reward_model_trainer=MagicMock(),
                     query_schedule_cls=MagicMock(),
                     pb_step_freq=100)


def test_should_derive_reward_training_frequency_from_pb_step_freq_if_not_provided(agent):
    pb_step_freq = 100
    agent = PbRLAgent(policy_model=MagicMock(),
                      reward_model=MagicMock(),
...
def test_is_training_step_after_correct_number_of_steps():
    agent = PbRLAgent(policy_model=MagicMock(),
                      reward_model=MagicMock(),
                      pretraining_query_generator=MagicMock(spec=AbstractQueryGenerator),
                      query_generator=MagicMock(),
...
def test_is_training_step_if_more_than_specified_number_of_steps_have_passed():
    agent = PbRLAgent(policy_model=MagicMock(),
                      reward_model=MagicMock(),
                      pretraining_query_generator=MagicMock(spec=AbstractQueryGenerator),
                      query_generator=MagicMock(),
...
def test_should_set_last_training_timestep_to_current_after_training():
    agent = PbRLAgent(policy_model=MagicMock(),
                      reward_model=MagicMock(),
                      pretraining_query_generator=MagicMock(spec=AbstractQueryGenerator),
                      query_generator=MagicMock(),
...
def test_should_train_reward_model_if_more_than_specified_number_of_steps_have_passed():
    agent = PbRLAgent(policy_model=MagicMock(),
                      reward_model=MagicMock(),
                      pretraining_query_generator=MagicMock(spec=AbstractQueryGenerator),
                      query_generator=MagicMock(),
...
def test_should_not_train_reward_model_if_not_enough_steps_have_passed():
    agent = PbRLAgent(policy_model=MagicMock(),
                      reward_model=MagicMock(),
                      pretraining_query_generator=MagicMock(spec=AbstractQueryGenerator),
                      query_generator=MagicMock(),
