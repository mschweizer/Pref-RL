...
from query_generation.segment_queries.segment_sampler import AbstractSegmentSamplerMixin
from query_generation.segment_queries.segment_sampling_callback import SegmentSamplingCallback


@patch.multiple(AbstractSegmentSamplerMixin, __abstractmethods__=set())
def test_samples_trajectory_segment_every_sampling_interval(policy_model):
    sample_mock = Mock()
    interval = 10

    segment_sampler = AbstractSegmentSamplerMixin(segment_samples=[],
